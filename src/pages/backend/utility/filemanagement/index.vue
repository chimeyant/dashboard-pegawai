<template>
  <v-row :class="device.mobile || device.tablet ? `` :`mx-1 mt-1`">
        ...
        <v-infinite-scroll :onLoad="load">
          ...
          >
            <div :class="['pa-2', index % 2 === 0 ? 'bg-grey-lighten-2' : '']">
              <v-row>
                <v-col cols="10">
                  <span class="font-weight-bold text-body-2">File :{{ item.file_name }}</span><br>
                  <span class="text-body-2">Folder : {{ item.folder_name }}</span><br>
                  <span class="text-body-2">{{ item.size }} Kb</span>
                </v-col>
                <v-spacer></v-spacer>
    ...
    <div>
      ...
      >
        <div>
          <div class="title font-weight-bold">Cara Pengguna Modul Manajemen Pengguna</div>
          <br>
          <div>
            <p class="text-body-2">
              Modul ini berfungsi untuk melakukan manajemen pengguna antara lain untuk melakukan penambahan data, perubahan dan penghapusan
            </p>
          </div>
        </div>
      </form-helper>
...
export default {
  ...
  setup() {
    ...
    const endpoint = "api/v1/utility/file-managements";

    const winheight = window.innerHeight - 320;
    const headers = [
      { title: "Nama File", key: "file_name", align: "start" },
      { title: "Folder", key: "folder_name", align: "start" },
      { title: "Ektensi", key: "ext", align: "start" },
      { title: "Ukuran (kb)", key: "size", align: "start" },
      {
        title: "AKSI",
        key: "id",
        align: "center",
    ...
    onMounted(() => {
      store.setPage({
        title: "Manjemen File",
        subtitle: "Berikut Daftar File yang ada dalam aplikasi",
        actions: {
          refresh: true,
          add: false,
          edit: false,
