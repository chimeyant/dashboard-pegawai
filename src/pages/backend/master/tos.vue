  ...
  <v-row :class="device.mobile || device.tablet ? `` :`mx-1 mt-1`">
    <v-col cols="12">
      <frame-table
        ...
        @btnBack="router.push({name:'master-section'})"
        ...
        >
          ...
          <template v-slot:item.id="{item}">
            <v-menu location="start">
              ...
              >
              <v-list-item
                  title="Atur Grup Pelayanan"
                  @click="openGroupService(item.id)"
                >
    ...
    <div>
      ...
      >
        <div>
          <div class="title font-weight-bold">Modul Kategori</div>
          <br>
          <div>
            <p class="text-body-2">
              Modul ini berfungsi untuk melakukan penambahan, perubahan dan penghapusan data kategori
            </p>
          </div>
        </div>
      </form-helper>
...
export default {
  ...
  setup() {
    ...
    const endpoint = "api/v2/master-data/tos/"+ route.params.section_uuid+"/";

    const winheight = window.innerHeight - 320;
    const headers:{}[] = [
      { title: "KATEGORI", key: "name", align: "start" },
     ...
     *
     */
    const openGroupService = (payload:any)=>{
      router.push({
        name:"master-group-service",
        params:{
    ...
    onMounted(() => {
      store.setPage({
        ...
        subtitle: "Berikut Daftar Seluruh Kategori Pelayanan Yang Tersedia ",
        dataUrl: endpoint,
        actions: {
          refresh: true,
          add: true,
