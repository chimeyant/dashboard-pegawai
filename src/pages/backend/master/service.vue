...
export default {
  ...
  setup() {
    ...
    const endpoint =
      "api/v2/master-data/services/" + route.params.section_uuid + "/"+ route.params.tos_uuid+"/"+ route.params.group_service_uuid+"/";

    const winheight = window.innerHeight - 320;
    const headers:{}[] = [
      { title: "NAMA PELAYANAN", key: "name", align: "start" },
    ...
    onMounted(() => {
      store.setPage({
        ...
        subtitle:
          "Berikut Daftar Seluruh Pelayanan Yang Tersedia Dalam Bidang Ini ",
        dataUrl: endpoint,
        actions: {
          refresh: true,
          add: true,
