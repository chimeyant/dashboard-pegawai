<template>
  <v-row :class="device.mobile || device.tablet ? `` :`mx-1 mt-1`">
    ...
    <v-col cols="12">
      <frame-form
        title="Formulir Jenis Berita Acara Pemeriksaan"
        width="600"
        @btnAdd="postRecord"
        @btnEdit="postUpdate"
    ...
    <div>
      ...
      >
        <div>
          <div class="title font-weight-bold">Modul Jenis Berita Acara Pemeriksaan</div>
          <br>
          <div>
            <p class="text-body-2">
              Modul ini berfungsi untuk melakukan penambahan, perubahan dan penghapusan data jenis berita acara pemeriksaan
            </p>
          </div>
        </div>
      </form-helper>
...
export default {
  ...
  setup() {
    ...
    const endpoint = "api/v1/master-data/jenis-berita-acara-pemeriksaan/";

    const winheight = window.innerHeight - 320;
    const headers: {}[] = [
      { title: "JENIS BERTIA ACARA", key: "name", align: "start" },
    ...
    onMounted(() => {
      store.setPage({
        title: "Jenis Berita Acara Pemeriksaan",
        subtitle:
          "Berikut Daftar Seluruh Jenis Berita Acara Pemeriksaan Yang Tersedia ",
        dataUrl: endpoint,
        actions: {
          refresh: true,
          add: true,
