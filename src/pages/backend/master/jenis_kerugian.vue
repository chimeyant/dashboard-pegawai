<template>
  <v-row :class="device.mobile || device.tablet ? `` :`mx-1 mt-1`">
    ...
    <v-col cols="12">
      <frame-form
        title="Formulir Jenis Kerugian Daerah"
        width="600"
        @btnAdd="postRecord"
        @btnEdit="postUpdate"
    ...
    <div>
      ...
      >
        <div>
          <div class="title font-weight-bold">Modul Jenis Kerugian Daerah</div>
          <br>
          <div>
            <p class="text-body-2">
              Modul ini berfungsi untuk melakukan penambahan, perubahan dan penghapusan data jenis informasi kerugian daerah
            </p>
          </div>
        </div>
      </form-helper>
...
export default {
  ...
  setup() {
    ...
    const endpoint = "api/v1/master-data/jenis-kerugian-daerah/";

    const winheight = window.innerHeight - 320;
    const headers: {}[] = [
      { title: "JENIS KERUGIAN", key: "name", align: "start" },
    ...
    onMounted(() => {
      store.setPage({
        title: "Jenis Informasi Kerugian Daerah",
        subtitle:
          "Berikut Daftar Seluruh Jenis Informasi Daerah Yang Tersedia ",
        dataUrl: endpoint,
        actions: {
          refresh: true,
          add: true,
