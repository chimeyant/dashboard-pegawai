<template>
  <v-row :class="device.mobile || device.tablet ? `` :`mx-1 mt-1`">
        ...
        <v-row>
          ...
          <v-col cols="12">
            <v-autocomplete
              ...
              variant="outlined"
              density="compact"
              v-model="record.surat_penagihan_uuid"
              hide-details="auto"
              :items="suratPenagihans"
            ></v-autocomplete>
          </v-col>
    ...
    <div>
      ...
      >
        <div>
          <div class="title font-weight-bold">Modul SKTJM</div>
          <br>
          <div>
            <p class="text-body-2">
              Modul ini berfungsi untuk melakukan penambahan, perubahan dan penghapusan SKTJM
            </p>
          </div>
        </div>
      </form-helper>
...
import { formatToRupiah } from "@/helpers/utility";
...
export default {
  ...
  setup() {
    ...
    const formattedRupiah = computed(() =>
      formatToRupiah(record.value.nilai_setoran)
    );
    const endpoint = "api/v1/sidakuda/sts/";
    ...
    const headers: {}[] = [
      ...
      { title: "Nilai", key: "nilai_setoran", align: "end", width: 250 },
      {
        title: "Aksi",
        key: "id",
        align: "center",
    ...
    const fetchSuratPenagihans = async () => {
      try {
        const endpoint = "api/v1/combo/surat-penagihan";
        const result = await store.http.get(endpoint);
        suratPenagihans.value = result.data.data;
      } catch (error) {
        store.catchError(error);
      }
    };

    const onInputNilai = (event: Event) => {
      const target = event.target as HTMLInputElement;
      record.value.nilai_setoran = target.value.replace(/[^0-9]/g, "");
    };

    onMounted(() => {
      store.setPage({
        title: "STS",
        subtitle: "Berikut Daftar Seluruh STS Yang Tersedia ",
        dataUrl: endpoint,
        actions: {
          refresh: true,
          add: true,
