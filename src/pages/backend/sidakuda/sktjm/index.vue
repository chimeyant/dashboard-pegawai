    ...
    <v-col cols="12">
      ...
      >
        <v-row>
          ...
          <v-col cols="12">
            <v-select
              label="Nomor Penugasan Penuntutan"
              variant="outlined"
              density="compact"
              v-model="record.penerimaan_penolakan_uuid"
              hide-details="auto"
            ></v-select>
          </v-col>
...
export default {
  ...
  setup() {
    ...
    const formattedRupiah = computed(() => formatToRupiah(record.value.nilai));
    const endpoint = "api/v1/sidakuda/sktjm/";

    const winheight = window.innerHeight - 320;
    const headers: {}[] = [
      { title: "Nomor", key: "nomor", align: "start" },
      { title: "Nomor Penugasan Penuntutan", key: "nomor_pp", align: "start" },
      { title: "Nama", key: "name", align: "start" },
      { title: "Tanggal", key: "tanggal", align: "center" },
      { title: "Dokumen", key: "docfile", align: "center" },
      { title: "Nilai", key: "nilai", align: "end" },
      {
        title: "Aksi",
        key: "id",
        align: "center",
    ...
    const onInputNilai = (event: Event) => {
      ...
      record.value.nilai = target.value.replace(/[^0-9]/g, "");
    };

    onMounted(() => {
      store.setPage({
        title: "SKTJM",
        subtitle: "Berikut Daftar Seluruh SKTJM Yang Tersedia ",
        dataUrl: endpoint,
        actions: {
          refresh: true,
          add: true,
