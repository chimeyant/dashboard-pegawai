            ...
            <v-menu location="start">
              ...
              >
                <v-list-item
                  ...
                  :color="theme.color"
                  @click="openDaftarPelamarPage(item.id)"
                >
                ...
                <v-row class="justify-end align-center pr-5">
                  <v-menu location="start">
                    ...
                    <v-list
                      ...
                      nav
                    >
                      <v-list-item title="Manajemen Formasi">
                        <template v-slot:prepend>
                          <v-icon
                            :color="theme.color"
                            icon="mdi-hand-heart"
...
export default {
  ...
  setup() {
    ...
    const endpoint = "api/v1/main-menu/data-event/";
    const winheight = window.innerHeight - 320;

    const headers: {}[] = [
      { title: "KEGIATAN", key: "name", align: "start", sortable: false },
      {
        title: "JML. FORMASI",
        key: "jml_formasi",
        align: "end",
      ...
      { title: "WAKTU", key: "waktu", align: "center", sortable: false },
      {
        title: "AKSI",
        key: "id",
        align: "center",
     ...
     */
    const openDaftarPelamarPage = (payload: any) => {
      router.push({
    ...
    const exportDataPelamar = async () => {
      try {
        const result = await store.http.get(
          "api/v1/main-menu/export-data-pelamar"
        );
        ...
        XLSX.utils.book_append_sheet(wb, dataxls, "Daftar Pelamar"); // sheetAName is name of Worksheet
        //XLSX.utils.book_append_sheet(wb, pokemonWS, "pokemons");

        // export Excel file
        XLSX.writeFile(wb, "daftar-pelamar.xlsx");
      } catch (error) {}
    };
    ...
    onMounted(() => {
      store.setPage({
        title: "Data Verifikasi Kegiatan",
