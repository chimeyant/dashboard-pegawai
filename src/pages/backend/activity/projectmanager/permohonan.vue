      ...
      >
        ...
        >
          ...
          <template v-slot:item.priority={item}>
            <v-chip
              size="small"
              :color="item.priority? item.priority.color : 'grey'"
            >{{ item.priority ? item.priority.text :'NA'  }}</v-chip>
          </template>
...
export default {
  ...
  setup() {
    ...
    const endpoint = "api/v2/activity/project-manager/task-manager/";

    const winheight = window.innerHeight - 320;
    const headers: {}[] = [
      { title: "Waktu", key: "tanggal", align: "center", width: 150 },
      { title: "Judul", key: "name", align: "start" },
      {
        title: "Dari",
        key: "dari.name",
        align: "center",
    ...
    const openAction = (payload: any) => {
      router.push({
        name: "activity-project-manager-action",
        params: {
          task_header_uuid: payload,
        },
      });
    };
