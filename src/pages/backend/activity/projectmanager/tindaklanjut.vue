  ...
  <v-row :class="device.mobile || device.tablet ? `` :`mx-1 mt-1`">
    <v-col :cols="showprops ?`8`:`12` ">
      <frame-table
        ...
        @btnBack="router.push({name:'activity-project-manager-taskmanager'})"
    ...
    >
      <v-card
        class="fill-height animate__animated animate__fadeInRight"
        rounded="0"
      >
              ...
              <v-row>
                <v-col cols="12">
                  ...
                  <v-select
                    ...
                    label="Pilih TA atau Support"
                    variant="outlined"
                    density="compact"
                    :color="theme.color"
                    v-model="taskheader.record.assign_to"
                    :items="users"
                    hide-details
                  ></v-select>
                </v-col>
        ...
        <v-btn
          style="text-transform:none; bottom:10px;right:10px; position: absolute;"
          :color="theme.color"
          variant="flat"
          @click="postAssignTo"
        >
    ...
    <div>
      ...
      >
        <div>
          <div class="title font-weight-bold">Modul History Permohonan</div>
          <br>
          <div>
            <p class="text-body-2">
              Modul ini merupakan modul yang berfungsi untuk memonitor perkembangan permohonan per setatus perkerjaan
            </p>
          </div>
        </div>
      </form-helper>
...
export default {
  ...
  setup() {
    ...
    const postAssignTo = async () => {
      const result: any = await store.postRecord(
        "/api/v2/activity/project-manager/assign-to",
        {
          task_header_uuid: taskheader.value.record.id,
          user_id: taskheader.value.record.assign_to,
        }
      );

      if (result.data.success) {
        store.setSnackbar(result.data.message, theme.value.color);
        router.push({ name: "activity-project-manager-taskmanager" });
      }
    };
